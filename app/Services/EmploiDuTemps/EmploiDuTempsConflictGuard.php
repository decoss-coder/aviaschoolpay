        ...
        return DB::transaction(function () use ($data) {
            ...
            if ((clone $base)->where('classe_id', $data['classe_id'])->exists()) {
                throw ValidationException::withMessages([
                    'classe_id' => 'Cette classe a déjà un cours sur ce créneau.',
                ]);
            }

            if (!empty($data['enseignant_id'])
                && (clone $base)->where('enseignant_id', $data['enseignant_id'])->exists()) {
                throw ValidationException::withMessages([
                    'enseignant_id' => 'Cet enseignant est déjà occupé sur ce créneau.',
                ]);
            }
