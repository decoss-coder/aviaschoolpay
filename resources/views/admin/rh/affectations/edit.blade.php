...
@section('page-subtitle', 'Mettre à jour la classe, la discipline et le volume horaire')
...
<div class="max-w-5xl mx-auto">
    <form method="POST" action="{{ route('admin.rh.affectations.update', $affectation) }}" class="space-y-6">
        ...
        <div class="rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 text-sm text-blue-900">
            <b>Important :</b> ici, on ne crée pas une nouvelle matière. On précise seulement quelle discipline déjà rattachée à l’enseignant est affectée à une classe.
        </div>
                ...
                <div>
                    ...
                    <select name="enseignant_id" id="enseignant_id" required class="w-full px-3 py-2.5 bg-white border border-brand-100 rounded-xl text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-100 outline-none transition-all shadow-sm cursor-pointer">
                        @foreach($enseignants as $enseignant)
                            <option value="{{ $enseignant->id }}" data-disciplines="{{ e($enseignant->specialite ?? '') }}" @selected(old('enseignant_id', $affectation->enseignant_id) == $enseignant->id)>
                ...
                <div>
                    ...
                    <select name="matiere_id" id="matiere_id" required class="w-full px-3 py-2.5 bg-white border border-brand-100 rounded-xl text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-100 outline-none transition-all shadow-sm cursor-pointer">
                        @foreach($matieres as $matiere)
                            <option value="{{ $matiere->id }}" data-matiere-name="{{ e($matiere->nom) }}" @selected(old('matiere_id', $affectation->matiere_id) == $matiere->id)>{{ $matiere->nom }}</option>
                        @endforeach
                    </select>
                    <p id="discipline-help" class="text-[11px] text-gray-500 mt-1">La liste est filtrée selon les disciplines de l’enseignant quand elles correspondent aux matières paramétrées.</p>
                </div>
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-white via-white to-gold-50/40 rounded-2xl border border-gold-200/60 shadow-card-gold p-6">
            ...
            <div class="relative grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="0.5" min="0.5" max="60" name="volume_horaire_hebdo" value="{{ old('volume_horaire_hebdo', $affectation->volume_horaire_hebdo) }}" class="w-full px-3 py-2.5 pr-12 bg-white border border-gold-200 rounded-xl text-sm font-bold focus:border-gold-400 focus:ring-2 focus:ring-gold-100 outline-none transition-all shadow-sm">
